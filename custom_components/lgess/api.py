...
class LGESSApi:
    ...
    async def _login(self)->None:
        try:
            async with self._session.put(f"{self.base_url}/v1/login", ssl=False, json={"password":self._password}, timeout=10) as response:
    ...
    async def _request(self, endpoint:str, payload:dict[str,Any]|None=None)->dict[str,Any]:
        ...
        if payload:
            body.update(payload)
        try:
            async with self._session.post(self.base_url+endpoint, ssl=False, json=body, timeout=10) as response:
                response.raise_for_status()
                data=await response.json()
        except ClientError as err:
            raise LGESSApiError(err) from err
    ...
    async def async_get_graph_day(self, day:date)->dict[str,Any]:
        return await self._request("/v1/user/graph/pv/day",{"year_month_day":day.strftime("%Y%m%d")})
